...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Poxa uma pessoas saiu do grupo :( @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Poderia esperar um pouquinho amiguinho ou amiguinha?',
				success: 'Pronto😎👍',
				error: {
					stick: 'Infelizmente não consegui fazer sua figurinha:(',
					Iv: 'Este link é invalido'
				},
				only: {
					group: '❌ Este comando só funciona em grupo❌',
					ownerG: 'Este Comando só funciona com os adm do grupo😑'
					ownerB: '❌ Este comando só funciona com dono do bot❌',
					admin: 'Este comando só funciona com os adm do bot',
					Badmin: 'Para utilizar esse comando o bot precisa ser adm do grupo'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5516999916885@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'setprefix':
					...
					prefix = args[0]
					reply(`Prefixo foi alterado com sucesso : ${prefix}`)
					break
				/*case 'loli':
				...
				case 'stalk':
					try {
						...
						console.log(`Error :`, color(e,'red'))
						 reply ('Nome de usuario incorreto')
					}
					break
			
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('Oque quer escrever?')
					teks = body.slice(7)
					reply(mess.wait)
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'clearall':
					if (!isOwner) return reply('Quem usou este comando?') 
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('O chats foram apagados com sucesso')
					break
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Ecreva o numero certo')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Nao consegui add pois o número é privado'' )
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Alguem foi expulso:(')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Que pena, foi expuslo:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
...
//AQUI NAO PRECISA MUDAR                   
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Nao foi possivel converter a imagem')
